...
useHead({
  title: 'JSON Diff Comparator - Free Online Tool | AgentsAITools',
  meta: [
    { name: 'description', content: 'Compare two JSON objects. Free online tool by AgentsAITools - no signup required. Fast, secure, and easy to use.' },
    { name: 'keywords', content: 'json diff, json compare, json difference, online tool, free tool, developer tool' },
    { property: 'og:title', content: 'JSON Diff Comparator - Free Online Tool | AgentsAITools' },
    { property: 'og:description', content: 'Compare two JSON objects. Free tool, no signup required.' },
    { property: 'og:url', content: 'https://agentsaitools.com/json-diff' },
  ],
  link: [{ rel: 'canonical', href: 'https://agentsaitools.com/json-diff' }],
});
const faqJsonLd = { '@context': 'https://schema.org', '@type': 'FAQPage', 'mainEntity': [
  { '@type': 'Question', 'name': 'What is JSON Diff Comparator?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Compare two JSON objects. It is a free online tool that works directly in your browser.' } },
  { '@type': 'Question', 'name': 'How to use JSON Diff Comparator?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Simply enter your input data and the tool will process it instantly. No signup or installation required.' } },
  { '@type': 'Question', 'name': 'Is JSON Diff Comparator free?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Yes, JSON Diff Comparator is completely free to use. No signup required. All processing happens in your browser.' } },
]};
function goToTool() { router.push('/json-diff'); }
</script>
<template>
  <div class="landing-page">
    <script type="application/ld+json">{{ JSON.stringify(faqJsonLd) }}</script>
    <div class="hero"><h1>JSON Diff Comparator</h1><p class="subtitle">Compare two JSON objects</p><button class="cta-button" @click="goToTool">Use Tool →</button></div>
    ...
    <div class="faq"><h2>FAQ</h2><div class="faq-item"><h3>What is JSON Diff Comparator?</h3><p>Compare two JSON objects.</p></div><div class="faq-item"><h3>Is it free?</h3><p>Yes, completely free. No signup required.</p></div></div>
